"""Microsoft SQL Server dialect helpers."""
...
def normalize_dsn(dsn: str) -> str | None:
    if dsn.startswith("mssql://") and "+pyodbc" not in dsn.split(":", 1)[0]:
        return "mssql+pyodbc://" + dsn[len("mssql://") :]
    if dsn.startswith("sqlserver://") and "+pyodbc" not in dsn.split(":", 1)[0]:
        return "mssql+pyodbc://" + dsn[len("sqlserver://") :]
    return None
