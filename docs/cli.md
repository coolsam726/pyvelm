...
| `PYVELM_DATABASES` | — | Optional **preview** — comma `key=dsn` or JSON list for multi-DB routing (not v1.1; see [multi-database.md](multi-database.md)). |
...
pyvelm migrate --database tenant_a  # preview: target a PYVELM_DATABASES tenant
